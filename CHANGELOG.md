...
- **Refuses rather than substitutes, by construction, not by configuration.** Every billed-credential env var this package already knows about is stripped from the child process's environment before every invocation — not left unset by omission, deleted from a copy of the parent env. As shipped this covered only `ANTHROPIC_API_KEY`/`ANTHROPIC_API_KEY_FILE`; a review found `ANTHROPIC_AUTH_TOKEN` missing (this package's own `anthropic/client.ts` already treats it as a live credential vector for the same provider) and it is now stripped too, alongside two precautionary strips (`CLAUDE_CODE_USE_BEDROCK`, `CLAUDE_CODE_USE_VERTEX`) for enterprise routing the CLI is reported, but not independently verified here, to support. `route` is reported as the constant `'subscription-oauth'` — a deliberate design choice given this adapter has no fallback ladder, not a claim that every billed-credential path is provably closed. A non-zero exit throws; it never retries against a different credential. A caller-supplied `apiKey` is refused outright rather than silently ignored.
