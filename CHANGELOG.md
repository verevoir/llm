...
**A genuine concurrency defect, found by review before this wave first merged, and fixed in place rather than shipped and patched after.** `getOrCreateSession` used to check whether anything was held under a handle's id, then do real async work (killing a dead predecessor, `createToolBridge`'s own `mkdtemp`/`writeFile`/TCP-listen) before registering the new session — a classic check-then-act race. Two `runSessionTurn` calls fired back-to-back on the same fresh (or dead/evicted) handle both read the session map as empty, both spawned a full `claude` process plus tool bridge, and the second registration silently overwrote the first, orphaning its process, its bridge's listening socket, and its temp directory with no cleanup path ever able to reach them again — and falsifying this file's own documented "one turn in flight per session, refused rather than interleaved" claim for exactly that case. Fixed by reserving the handle's slot synchronously, before any `await`, so a concurrent call shares the in-flight spawn instead of racing a second one; pinned with a regression test that fires two calls without awaiting between them and asserts exactly one process spawns.
