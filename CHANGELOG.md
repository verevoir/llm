...
**Collapse `/deepseek` onto the shared OpenAI-compatible factory; add tool calling to `/openai`'s Responses API** (STDIO-342 continued — factoring correction).

Established from source: `/samba` and `/mistral` were already thin configuration over `createOpenAICompatAdapter`; `/deepseek` was not — it carried its own hand-maintained duplicate of the factory's Chat Completions request construction, `chat()`-only. That made the earlier tool-calling parity gap (SambaNova + Mistral had `chatWithTools`/`chatWithToolLoop`, `/openai` and `/deepseek` didn't) a **factoring defect** — one wire protocol implemented twice, once completely and once partially — not four providers with genuinely differing capability.

- **`/deepseek` rebuilt as configuration over the factory** — a model catalog + one `createOpenAICompatAdapter(...)` call, the same shape as `/samba`/`/mistral`, replacing the hand-written `callChatCompletionsCreate`/retry/usage-shaping code. `chatWithTools`/`chatWithToolLoop` fall out of the collapse rather than being written fresh. Every existing `./deepseek` export (`chat`, `PROVIDER`, `BASE_URL`, `models`, `rates`, the `deepseek` namespace) keeps its exact prior signature — nothing removed, nothing deprecated, purely additive. **Side effect:** the hand-written path had never picked up the factory's STDIO-487 fix (0.17.0) for a cached-token double-count — `inputTokens` included the cached subset instead of excluding it, over-billing every DeepSeek call that hit the prompt cache. Adopting the factory fixes this as a consequence of removing the duplication, not as a separate change.
- **`/openai` gains `chatWithTools` + `chatWithToolLoop`**, but NOT via the factory: `/openai` deliberately speaks OpenAI's Responses API (`responses.create`), a materially different wire shape from the Chat Completions API every OpenAI-compatible clone (DeepSeek, SambaNova, Mistral, and — under this same reasoning — Qwen, which needs no dedicated adapter, only configuration against this same seam) speaks. Tool declarations are flat (`{type:'function', name, ...}`, not nested under `function:{...}`); calls arrive as `function_call` items on `response.output`, not a `tool_calls` array; results are fed back as `function_call_output` items keyed by the provider's own `call_id`. This divergence is real — not tidied away — and is implemented as its own tool-calling path against that shape. Tool-call ids are always the provider's genuine `call_id`, never a name-derived fallback (`id: f.id ?? f.name` — Gemini's fallback, recorded as a parallel-call collision defect in `#47`, and independently fixed on `/google` itself in `#48`/0.26.4 with a proper per-response, per-iteration index — not the pattern here regardless). On iteration-cap-hit, one final no-tools call forces a written answer from the history, matching Anthropic / Gemini / the OpenAI-compatible factory. `onProgress` is untouched — still Anthropic-only across every route.
- **`/openai` and `/deepseek` are not touched together by accident** — the divergence between them is now explicit and load-bearing: `/deepseek` had no reason to differ and now doesn't; `/openai` has a real reason (a different vendor API surface) and keeps it.
- **Tool output re-entering model context:** both routes' tool loops re-send the executor's return value verbatim on the next iteration — `/deepseek` via the factory's existing `messages.push({ role: 'tool', ... })` path (unchanged by this PR), `/openai`'s new loop via `items.push({ type: 'function_call_output', ... })`. Noted for the concurrent prompt-injection sweep; no sanitisation is added here.
- **Timeout-contract coverage, as merged:** the concurrent `#51` (generic `runWithTimeoutContract` wrap, reassigned to 0.28.0 after a version-number collision) targets whatever entry points existed on `main` at the time it merges. See this PR's own body for the explicit, as-merged statement of which of `/openai`'s and `/deepseek`'s entry points (including the two new ones added here) ended up inside that wrap and which did not — stated there rather than here because it depends on merge order, not on this change's own content.
