...
- **Every other SDK-driven route — `openai`, `deepseek`, `google`, and the shared OpenAI-compatible factory behind `mistral`/`samba` — gets the CHEAP, UNIVERSAL half in this same release**: `chat`/`chatWithTools`/`chatWithToolLoop` are wrapped in `runWithTimeoutContract`, so an await on any of them rejects with the identical `code`/`timeoutMs` shape at the same bound, with zero vendor-specific knowledge — the same property that makes this satisfiable for Gemini without ever having read its SDK default. This does **not** stop the underlying vendor call: racing past a promise doesn't cancel it, so on these routes the real request keeps running in the background, discarded, until it resolves or errors on its own — a real resource/cost exposure, and a route whose `onProgress`/`onRetry` callbacks can still fire after the caller has already moved on. `TIMEOUT_TEARDOWN_CONFIRMED` is the single, checkable source of truth for which routes have closed that remaining gap; it grows by exactly one entry per route's own bespoke PR, in the same commit as the fix.
