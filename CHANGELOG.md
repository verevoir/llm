...
**One timeout contract across every route** — closes the keyed/keyless asymmetry where a caller could not write one piece of error handling that recognised "this turn wedged" no matter which route served the call. Before this, claude-cli's session-holding transport already had a real package-owned watchdog (`SESSION_TURN_TIMEOUT_MS`, 10 min, kills the process and rejects with a bespoke, testable error) but the keyed routes (Anthropic, OpenAI, DeepSeek, the OpenAI-compatible factory) never passed a `timeout` to their SDK client constructors at all — both `@anthropic-ai/sdk` and `openai` declare a ~10-minute-per-attempt default with `maxRetries: 2`, so the real worst case was silently nearer 30 minutes, and an ordinary `npm update` could change either number with no test noticing.

**Gemini turned out worse, not merely unmeasured.** Reading the installed `@google/genai@2.21.0` runtime directly (not its declarations) shows that with `httpOptions.timeout` unset — this adapter's actual configuration — the SDK builds no `AbortController` at all. There is no default timeout of any kind; a hung Gemini call never times out on its own and hangs until the process is killed or the caller's own `abortSignal` fires. That finding is why the generic half of this contract ships to every SDK-driven route in this SAME release rather than waiting for a later fan: under the requirement that every route stay interchangeable, a route that can hang forever cannot be left for later.

- **New core exports**: `LLM_CALL_TIMEOUT_MS` (10 min, matching `SESSION_TURN_TIMEOUT_MS` deliberately — the same wall-clock everywhere by default), `LLM_TIMEOUT_CODE` (`'LLM_TIMEOUT'`), `resolveTimeoutMs`, `makeTimeoutError`, `withTimeoutSignal`, `runWithTimeoutContract`, and `TIMEOUT_TEARDOWN_CONFIRMED`. `ChatOptions` gains an optional `timeoutMs` — retries and any provider-side fallback count INSIDE it, not just the caller's own await — refused synchronously (before any network/process work starts) when it is not a finite number greater than 0; `Infinity` is refused alongside `0`/negative/`NaN`, since accepting it would mean "the contract doesn't apply to me".
- **The error shape is a field, not a class or a message match**: every route throws a plain `Error` carrying `code: 'LLM_TIMEOUT'` and `timeoutMs` as own, enumerable properties — matching how a thrown SDK error is already classified elsewhere in this estate (structural-field inspection, never `instanceof`) and avoiding the exact fragility already disclosed against claude-cli's own message-prefix matches. In the five-tag outcome vocabulary built on this package, this maps to `unreachable` — never `refused` (a definite, reached-and-declined outcome) or `invalid` (something came back and couldn't be parsed): a timeout has no definite outcome, and the attempt was torn down deliberately. This also **corrects a real, pre-existing misclassification** one layer up: an Anthropic hang previously surfaced as a status-less throw a downstream consumer could only read as `invalid`, nothing else distinguishing it; it now carries a `code` a consumer can read correctly as `unreachable`. That reclassification lives in each consumer's own adapter code, not in this package — this release only makes the correct classification possible.
- **Anthropic is the exemplar, with genuine teardown, not just shape.** `anthropic/client.ts` now sets `maxRetries: 0` and an oversized `timeout` (24h) on every client it constructs — not merely alongside our own watchdog, but INSTEAD of relying on the vendor's: picking our own bound to equal the SDK's un-neutralised default (both happen to be 10 minutes) would otherwise race the two timers against each other, decided by whichever socket-level tick wins. `anthropic/index.ts` threads the contract's combined `AbortSignal` into the live `client.messages.stream(request, { signal })` call across `chat`, `chatWithTools`, and `chatWithToolLoop` — including the OAuth→API-key fallback and every tool-loop iteration — so an expired timeout genuinely tears the in-flight request down, the SDK-request analogue of claude-cli's own `child.kill()`. `TIMEOUT_TEARDOWN_CONFIRMED` is seeded with `'anthropic'` in this release.
- **`google` and the shared OpenAI-compatible factory (`mistral`/`samba`) get the CHEAP, UNIVERSAL half on all three entry points in this same release**: `chat`/`chatWithTools`/`chatWithToolLoop` are wrapped in `runWithTimeoutContract`, so an await on any of them rejects with the identical `code`/`timeoutMs` shape at the same bound, with zero vendor-specific knowledge — the same property that makes this satisfiable for Gemini without ever having read its SDK default. **`openai` and `deepseek` get the wrap on `chat()` only** — neither route had a `chatWithTools`/`chatWithToolLoop` export on `main` at this PR's base commit (`2e3b485`), so there was nothing else to wrap; a concurrent PR (`#49`) adds those exports to `openai` and rebuilds `deepseek` onto the same factory `mistral`/`samba` already use, and the wrap needs extending to them once that lands. None of this stops the underlying vendor call: racing past a promise doesn't cancel it, so on these routes the real request keeps running in the background, discarded, until it resolves or errors on its own — a real resource/cost exposure, and a route whose `onProgress`/`onRetry` callbacks can still fire after the caller has already moved on. `TIMEOUT_TEARDOWN_CONFIRMED` is the single, checkable source of truth for which routes have closed that remaining gap; it grows by exactly one entry per route's own bespoke PR, in the same commit as the fix.
- **`claude-cli` is deliberately untouched by this release, in either half.** `src/claude-cli/session.ts` and its consumer `src/claude-cli/index.ts` are under active, same-day development on a concurrent branch (#45, a session-holding transport); editing either file here risked colliding with work already in flight. Its existing `abortSignal`-driven teardown (`child.kill()` on abort, already covered by a non-vacuous test) is untouched and still correct — but until a follow-up PR wires `options.timeoutMs` through it and attaches `code`/`timeoutMs` to the error it throws on abort, a caller who sets `timeoutMs` on `claudeCli.chat()` today has that option **silently ignored**, not refused — exactly the kind of silent asymmetry this contract exists to remove, named here rather than left for someone to discover.
- **A merge with `#49` (open, concurrent, additive — adds `chatWithTools`/`chatWithToolLoop` to `openai`, rebuilds `deepseek` onto the shared factory) is expected to be clean, but clean is not the same as correct**: those new exports would land unwrapped, with no conflict to flag it. `src/timeout-contract-coverage.test.ts` closes that specific risk with an explicit, maintained table of every `chat`-family entry point and its coverage status, plus a check that fails the build the moment the table and reality diverge in either direction — an export appears that isn't listed, or a listed entry no longer exists. A behavioural assertion (not a construction check) proves every `covered: true` row is genuinely wrapped: `timeoutMs: 0` is refused before any credential/client work runs, on every one of them. The table is deliberately explicit rather than structurally inferred — a maintained list's omission risk is a human failure mode this estate does not have here, and an explicit table is legible in a way a clever inference is not; a second check anchors the route list itself against `package.json`'s own `exports` map so a new provider subpath can't go unrecorded either.
- **Not this package's concern, found by the same investigation and noted so it isn't mistaken for something this release fixes:** `ChatOptions.maxTokens` is also silently unread on `google`, `openai`, `deepseek`, and the OpenAI-compatible factory — a caller moving a `maxTokens` override from Anthropic to any of those gets silent truncation with no signal. Same defect class as the timeout asymmetry; out of scope here.
