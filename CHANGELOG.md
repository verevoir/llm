...
**SECURITY CORRECTION to this same unreleased change — the session-holding path shipped the identical `--tools ""` fail-open fixed separately on the single-shot path in 0.26.3.** `buildSessionArgs` passed `--tools ""` believing it disabled every built-in tool; per 0.26.3's own finding, an empty string is read as NOT SET, so it silently did nothing. Fixed to `--disallowedTools "*"`, the confirmed-working form — and, deliberately, this path's fix is NOT "zero tools" the way 0.26.3's single-shot path is: `chatWithToolLoop`'s whole purpose is the caller's OWN tools reaching the model, so this relies, ON PURPOSE, on a confirmed CLI bug (`--disallowedTools` not reliably filtering MCP-server tools) to deny every built-in by name while leaving the caller's MCP-declared tools (via the embedded bridge) reachable — recorded plainly at the flag-building site and in `session.ts`'s own file header, because an upstream fix to that CLI bug would break this adapter's tool-bearing sessions silently otherwise. **Confirmed against a real, authenticated, run-to-completion invocation** (the third operator-run probe): asked to actually invoke Bash rather than describe it, the model could not — zero `tool_use` blocks anywhere in the transcript, and both the CLI's own `init` event (`"tools":[]`) and the model's own refusal to fabricate a result agree. The MCP question this file has open (whether `--safe-mode` is what's keeping `mcp_servers` empty) is UNCHANGED by this fix and remains open — see `session.ts`'s own file header for the fourth, isolating probe this points to next.

**`claude-cli` gains a session-holding transport — feature parity on the keyless route was the bar, and `chat()`-only left every task on it blocked at the model call the moment it needed a tool.** `chatWithToolLoop`/`chatWithTools` didn't exist on this adapter at all; `chat()`'s `--tools ""` plus one process spawned and exited per call meant a tool exchange (ask → execute → continue) had structurally nowhere to live — turn N and turn N+1 were two different processes with nothing in common. Scoped first (this PR's own body has the full account of what was ruled out and why); the operator's decision that followed: hold a session across a whole conversation, not one call, and expose it as a handle the CALLER owns — this package has no concept of a conversation, so a lifetime keyed on one would be policy this package cannot reason about.

- **New `createSession()` / `closeSession()` — an opaque `ClaudeCliSessionHandle`.** Nothing about the session lives on the handle itself; everything is internal state keyed by its id. A handle presented after its process died, was evicted, or was simply never used is NOT an error — the next call transparently starts fresh under the same id, the same shape as a 401: the caller's own `turns` stay the ground truth, the session is only ever an accelerator that may be absent.
- **Bounded, so an unreleased handle cannot accumulate processes forever — and the bound is a resource policy, not a correctness one.** `SESSION_IDLE_TIMEOUT_MS` (5 min) measures the GAP between turns, never a turn's own length: it is cancelled the moment a turn starts and only rescheduled once it settles, so a single turn that legitimately runs long stays safe (bounded solely by `SESSION_TURN_TIMEOUT_MS` instead) while a session sitting quiet between turns — a self-paced loop idling for hours — is exactly what gets reclaimed. `SESSION_MAX_HELD` (4, LRU-evicted) is the second, independent bound. Both are exported and safe to tune freely in either direction: the session is a cache over the caller's own stateless `turns`, so getting the TTL wrong costs a token-spending cold rebuild, never lost work — the caller can always rehydrate through the stateless single-shot path (see `ClaudeCliChatOptions`'s own doc comment). A `chatWithToolLoop()` call that never names a `session` opens a throwaway one and closes it itself when the call returns, so a caller that doesn't want to hold state sees a fully self-contained call, same as every other adapter's.
- **New `chatWithToolLoop(options)`** — same outward `ChatWithToolLoopOptions`/`ChatWithToolLoopResult` shape as the other four adapters, with an additive optional `session`. Mechanism: `claude -p --input-format stream-json --output-format stream-json` is kept alive for the session's life; tools are exposed WITHOUT lifting the built-in-tool denial (`--disallowedTools "*"` as of the SECURITY CORRECTION bullet below — originally written, and shipped on this branch, as `--tools ""`) (the tools offered must be exactly the caller's, never Claude Code's own — an explicit, unmoved constraint) via a new embedded, per-session MCP stdio server (`mcp-bridge.ts`) named through `--mcp-config` (`--strict-mcp-config` stays on, so it's the only server reachable). That server forwards every `tools/call` over a loopback TCP hop back into this process, where the caller's own `ToolExecutor` actually runs — so `claude`'s own agentic loop calls tools autonomously within ONE turn request, rather than this function manually feeding `tool_use`/`tool_result` blocks back and forth the way the API adapters do. `maxIterations` is enforced as a tool-CALL budget inside the bridge — once spent, further calls get a "budget exhausted, answer now" result rather than reaching the executor. `iterations` on the result is a disclosed PROXY (the count of `assistant` stream events observed for the turn), not a count of separate model calls this function made (it makes exactly one).
- **`chatWithTools()` (single-shot, return-before-execution) REFUSES on this transport, naming precisely why, rather than faking the contract.** Tools run through the CLI's own agentic loop via MCP — the bridge answers a `tools/call` by running the executor immediately and synchronously, so there is no point in the exchange where a `tool_use` exists without already having been executed for a caller to run separately. Declares the gap rather than pretending: `chatWithToolLoop()` is the shape this transport can honestly support.
- **`chat()` gains an optional `session`.** With one set, `turns` must carry exactly the ONE new message for the round — the held session already remembers everything before it, unlike this adapter's stateless single-shot path (and every other adapter, which re-sends the full growing `turns` array every call). Held open across calls, the same session is what makes prompt-cache warmth possible on this route for the first time — tools and caching arrive together on this transport, not separately, because both need the same live process.
- **RELAYED, NOT CONFIRMED**, the same standard this adapter already holds `--model` to: whether `--input-format`/`--output-format stream-json` keeps one process alive across multiple turns, whether a turn's terminal event is genuinely `{"type":"result",...}` carrying the same inner shape as the confirmed single-shot envelope, and whether an MCP tool named via `--mcp-config` stays reachable under `--tools ""` are all asserted from Claude Code's own documented flags and MCP's own published stdio transport spec — not independently observed by this repository. `session.ts`'s own file header states the exact invocation needed to confirm or correct this. A per-turn watchdog (`SESSION_TURN_TIMEOUT_MS`, 10 min) kills the held process and refuses the turn with a specific, legible error rather than hanging if the real shape doesn't match; a malformed stream-json line is logged and skipped rather than treated as fatal on its own.
- **`--verbose` is now in `buildSessionArgs`, and this ONE flag is CONFIRMED, not relayed** — the operator ran the header's own probe invocation and it failed immediately, before producing any stream-json output at all: `Error: When using --print, --output-format=stream-json requires --verbose`, exit code 1. That is a real, first-party defect this adapter's flag list had — every production `runSessionTurn` call would have hit it — fixed here and pinned with a test asserting the built argument list always carries `--verbose` whenever `--output-format stream-json` does.
- **A second, corrected probe run (with `--verbose`) confirmed two of the three things the first run never got far enough to test — the third remains genuinely open.** The run failed on expired OAuth (`"Failed to authenticate: OAuth session expired and could not be refreshed"`, arriving IN-BAND on stdout's own `result` field with `stderr` EMPTY — itself a live illustration of why this adapter never trusts stderr/exit-code alone), so no model call ever reached the point of deciding whether to invoke a tool. But it ran to completion otherwise, and CONFIRMED: (1) one process serves MULTIPLE turns — two `{"type":"result",...}` lines, one shared `session_id`, distinguished by `result_index: 0`/`1`, no process exit between them; (2) the terminal envelope's shape matches exactly what `session.ts` parses it as (`result`/`is_error`/`subtype`/`stop_reason`/`usage`/`modelUsage` all present). STILL OPEN, not to be read either way from this run: whether an MCP tool stays reachable under `--tools ""` — both `init` lines showed `"tools":[],"mcp_servers":[]`, consistent with several different explanations the run can't distinguish (blocked by `--tools ""`, config never read, or the run dying before MCP connection), not favouring any one of them. Also surfaced, already handled correctly: the observed envelope carried `"subtype":"success"` ALONGSIDE `"is_error":true` — `subtype` describes how the request/response cycle ended, not whether the content is a failure, and `session.ts` was already reading only `is_error` as the failure signal, never `subtype`. Recorded, and now guarded with an inline comment at the check itself, because the two fields read as self-contradictory to a human at a glance and a future "fix" for that must not become reading `subtype` as the success signal instead.
- **Internal-only refactor, no export change:** the env allowlist (`ALLOWED_ENV_VARS`/`allowedEnv`/`CLAUDE_CLI_CREDENTIAL_ENV_VAR`) and the `PROVIDER` constant moved to a new `env.ts` so `session.ts` can build the identical child environment without a module cycle with `index.ts`. Every name still resolves from `@verevoir/llm/claude-cli` exactly as before, re-exported from the same entry point.
