...
interface PendingTurn {
  ...
  /** Consolidated teardown for THIS turn's watchdog + abort listener —
   * called from every one of the four settlement paths (result line,
   * watchdog fire, abort fire, process death) instead of each path
   * hand-removing the two separately. Fixed a real leak: the abort
   * listener used to be removed on only one of the four paths, so a
   * turn that resolved normally left it armed against a session that
   * was legitimately still open — a later abort on a reused
   * AbortController would then kill a healthy session. One teardown
   * point that every path goes through is the fix; four correct-looking
   * removals is the shape that produced the bug in the first place. */
  teardown: () => void;
}
...
export async function runSessionTurn(options: SessionTurnOptions): Promise<SessionTurnResult> {
  ...
  try {
    envelope = await new Promise<{
      ...
      // Consolidated teardown — see PendingTurn.teardown's own doc
      // comment for the bug this fixes: every settlement path (result,
      // watchdog, abort, process death) calls THIS, once, rather than
      // each hand-clearing the watchdog and removing the abort listener
      // separately, which is exactly the shape that once left the abort
      // listener armed against an already-resolved turn.
      const teardown = () => {
