...
describe('claude-cli session lifecycle', () => {
  ...
  describe('abortSignal', () => {
    ...
    it('result-then-abort on a reused AbortController does NOT kill the now-healthy session — the abort-listener-leak regression (#53)', async () => {
      const { child, emitLine } = fakeChild();
      mockSpawn.mockImplementationOnce(() => child);
      const session = createSession();
      const controller = new AbortController();
      ...
      // The SAME AbortController fires later — plausible if a caller
      // reuses one across a request whose lifetime outlives a single
      // turn. Before the fix, the 'abort' listener from the first turn
      // was never removed on the result path, so this would kill an
      // already-healthy, completed session.
      controller.abort(new Error('unrelated later abort'));

      expect(child.kill).not.toHaveBeenCalled();

      // The session is still usable for a second turn on the same process.
      const p2 = runSessionTurn({
        session,
        systemPrompt: 'sys',
        message: 'b',
      ...
      expect(r2.text).toBe('second');
      expect(mockSpawn).toHaveBeenCalledTimes(1); // no respawn — the session survived
    });
  });
});
