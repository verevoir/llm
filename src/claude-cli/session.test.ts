...
describe('claude-cli session lifecycle', () => {
  ...
  describe('abortSignal', () => {
    ...
    // reused AbortController. See the file header's KNOWN LIMITATION
    // paragraph — that scenario currently DOES incorrectly kill the
    // session, and the regression test proving both the bug and its fix
    // lands in the very next wave of this split, alongside the fix
    // itself, rather than being added here to document a defect this
    // wave isn't fixing.
  });
});
